...
// 개체값은 스탯당 -10 ~ +10이므로 4개 합계의 최대치는 40이다 (lib/stats.ts)
const MAX_TOTAL_IV = 40;
...
// currentStats에는 개체값 총합이 함께 온다
type FinalStats = Stats & { totalIv: number };
...
type UserMonster = {
  ...
  baseStats: Stats;              // 종족값 (도감에서 주로 사용)
  currentStats: FinalStats;      // 개체값이 반영된 실제 능력치
  pendingIv: PendingIv | null;   // 중복 획득으로 제안된 새 개체값
...
export default function MyMonstersPage() {
  ...
  const selected = monsters.find((m) => m.userMonsterId === selectedId) ?? null;
  ...
  const loadMonsters = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || body?.code !== CODE_OK) {
        setError(body?.message ?? '보유 몬스터를 불러오지 못했어요.');
        return;
      }
   ...
   */
  const handleIvDecision = async (userMonsterId: string, decision: 'accept' | 'reject') => {
    if (busy) return;
    setBusy(true);
    setError(null);

    try {
      const res = await fetch(`/api/user-monsters/${userMonsterId}/iv`, {
        method: 'POST',
      ...
      if (!res.ok || body?.code !== CODE_OK) {
        setError(body?.message ?? '개체값 처리에 실패했어요.');
        return;
      }
  ...
  const pendingCount = monsters.filter((m) => m.pendingStats?.totalIv != null).length;

  return (
    // 화면 높이를 고정하고 바깥 스크롤을 막아, 목록 영역에서만 스크롤되게 한다
    <div className={`${notoSans.className} h-[100svh] bg-[#F2F0E8] flex flex-col overflow-hidden`}>
      ...
      <div className="shrink-0 px-4 pt-6 pb-4">
        <div className="grid grid-cols-3 items-center">
          ...
          <h1 className="text-lg font-black text-center text-[#1B1B1B]">내 몬스터</h1>
      ...
      <div className="flex-1 min-h-0 bg-[#FDFCF8] mx-4 mb-4 rounded-2xl overflow-y-auto shadow-sm">
        <div className="px-4 py-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs tracking-widest text-[#B0BDB4] font-bold">MY MONSTERS</p>
              ...
              {SORT_OPTIONS.map((option) => (
                <button
                  ...
                  className={`px-2.5 py-1 text-xs font-bold rounded-lg transition-colors ${
                    sortKey === option.key ? 'bg-[#1F4B3C] text-white' : 'text-[#8A9A8E]'
                  }`}
                >
                  {option.label}
                </button>
          ...
          {!loading && pendingCount > 0 && (
            <div className="flex items-center gap-2 bg-[#FFF6E5] border border-[#F0DFBC] rounded-xl px-3 py-2.5 mb-4">
              <Sparkles size={14} className="text-[#C08A2E] shrink-0" />
              <p className="text-[11px] font-bold text-[#8A6520] leading-relaxed">
                {pendingCount}마리에게 새로운 개체값이 제안됐어요
              </p>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <p className="text-sm text-[#C0503D]">{error}</p>
              <button
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
              <div>
                <p className="text-sm font-bold text-[#4B5A50] mb-1">아직 수집한 몬스터가 없어요</p>
                <p className="text-xs text-[#8A9A8E]">주변 사물을 촬영해 첫 몬스터를 만나보세요</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-3 gap-2.5">
              {monsters.map((m) => (
                <button
                  key={m.userMonsterId}
                  onClick={() => setSelectedId(m.userMonsterId)}
                  className="rounded-2xl overflow-hidden shadow-sm text-left transition-transform active:scale-95 bg-white border border-[#EBE8DF] relative"
                >
      ...
      {selected && (
        ...
        >
          ...
          >
            {/* 이미지 영역 */}
            <div className="relative aspect-square overflow-hidden bg-[#181C19]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
              ...
              <button
                onClick={() => setSelectedId(null)}
                aria-label="닫기"
                className="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/50 backdrop-blur-md border border-white/20 flex items-center justify-center text-lg font-bold text-white leading-none active:scale-90 transition-transform"
              >
                ×
              </button>
            </div>
            ...
            <div className="px-5 pb-5 -mt-8 relative">
              ...
              {/* 능력치. 개체값이 반영된 실제 값과 종족값 대비 증감을 보여준다 */}
              <div className="bg-white/[0.06] border border-white/10 rounded-2xl p-3.5 mb-3">
                <div className="flex items-center justify-between mb-2.5">
                  <p className="text-[10px] font-extrabold tracking-wider text-white/60">능력치</p>
                  <span
                    className={`text-[10px] font-bold ${
                      selected.currentStats.totalIv >= 0 ? 'text-[#50C878]' : 'text-[#FF6B6B]'
                    }`}
                  >
                    개체값 {selected.currentStats.totalIv > 0 ? '+' : ''}
                    {selected.currentStats.totalIv} / {MAX_TOTAL_IV}
                  </span>
                </div>
                      ...
                      <div key={key} className="flex items-center justify-between">
                        ...
                        <div className="flex items-baseline gap-1.5">
                          ...
                          {diff !== 0 && (
                            <span
                              className={`text-[10px] font-bold ${
                                diff > 0 ? 'text-[#50C878]' : 'text-[#FF6B6B]'
                              }`}
                            >
                              {diff > 0 ? `+${diff}` : diff}
                            </span>
              ...
              {selected.pendingStats?.totalIv != null && (
                <div className="bg-[#1A1508] border border-[#4A3D18] rounded-2xl p-3.5">
                  <div className="flex items-center gap-1.5 mb-2.5">
                    <Sparkles size={13} className="text-[#E8B84B]" />
                    <p className="text-[11px] font-bold text-[#E8B84B]">새로운 개체값 제안</p>
                  </div>

                  <div className="space-y-1.5 mb-3">
                    {STAT_LABELS.map(([key, label]) => {
                      const current = selected.currentStats[key];
                      const proposed = selected.pendingStats![key];
                      ...
                      return (
                        <div key={key} className="flex items-center justify-between">
                          ...
                          <div className="flex items-baseline gap-1.5">
                            ...
                            <span className="text-sm font-black text-white">{proposed}</span>
                            {diff !== 0 && (
                              <span
                                className={`text-[10px] font-bold ${
                                  diff > 0 ? 'text-[#50C878]' : 'text-[#FF6B6B]'
                  ...
                  <div className="flex items-center justify-between mb-3 pt-2 border-t border-white/10">
                    <span className="text-[11px] text-white/50">개체값 총합</span>
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-xs text-white/40">
                        {selected.currentStats.totalIv}
                      </span>
                      <span className="text-xs text-white/30">→</span>
                  ...
                  <div className="flex gap-2">
                    <button
                      disabled={busy}
                      onClick={() => handleIvDecision(selected.userMonsterId, 'reject')}
                      className="flex-1 py-2.5 rounded-xl bg-white/10 text-white/70 font-bold text-xs border border-white/15 active:scale-95 transition-transform disabled:opacity-50"
                    >
                      그대로 두기
                    </button>
                    <button
                      disabled={busy}
                      onClick={() => handleIvDecision(selected.userMonsterId, 'accept')}
                      className="flex-1 py-2.5 rounded-xl bg-[#E8B84B] text-[#1A1508] font-black text-xs active:scale-95 transition-transform disabled:opacity-50"
                    >
                      {busy ? '처리 중...' : '교체하기'}
                    </button>
                  </div>
              ...
              {error && (
                <p className="mt-3 text-[11px] text-[#FF6B6B] text-center">{error}</p>
              )}
            </div>
          </div>
        </div>
